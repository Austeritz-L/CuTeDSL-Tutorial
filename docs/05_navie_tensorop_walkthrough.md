...
- 一个 CTA 如何映射到一个 m16n8k8 MMA atom？
...
教学版 GMEM->SMEM copy 仍然用普通 atom：

```python
g2S_copy_A = cute.make_copy_atom(cute.nvgpu.CopyUniversalOp(), ...)
g2S_copy_B = cute.make_copy_atom(cute.nvgpu.CopyUniversalOp(), ...)
```
...
```text
thread layout size = 16 * 2 = 32
A tile size        = 16 * 8 = 128
每线程 copy 4 个 fp16
```
...
```text
A tile = 16 x 8 = 两个 8 x 8
```

为什么 B 是 `.x1`：

```text
B tile = 8 x 8 = 一个 8 x 8
```
...
显示的是编译期 tensor type/layout。它通常只打印一次，因为它发生在 JIT tracing 阶段。
...
发生在 GPU kernel 执行时，会真实读取 tensor view。调试时要加 guard：
...
扩大 CTA tile
多个 warp 协作
GMEM->SMEM vectorized/cp.async
SMEM swizzle
double buffering / multi-stage
更多 MMA atom tiled together
```
