            ...
            Thread_LayoutB thread_layoutB)
{
   ...
   Tensor smem_buff_T = shared_storage.tensor_sA_T(); 
...
void test_transpose(TypeA const* device_ptr_A, LayoutA layout_A, TypeA* device_ptr_B)
{
    ...
    auto cta_tiler_T = make_shape(size<1>(cta_tiler), size<0>(cta_tiler));
    ...
    print_warp_banks(smem_base,     "base normal write", 0, false);
    print_warp_banks(smem_base_T,   "base transpose read", 0, true);
